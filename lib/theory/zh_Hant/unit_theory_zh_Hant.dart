...
// docs/research/ 中的研究檔案。

part of '../unit_theory.dart';

List<UnitTheorySection> _converterIntroZhHant() {
  return const [
    UnitTheorySection(
      '換算器能做什麼',
      '單位換算器在英制十二進位世界（基底十二）與公制十進位世界'
          '（基底十）之間進行換算——涵蓋所有類別：長度、面積、體積、'
          '重量、時間、角度、貨幣、溫度，以及壓力、力、能量、功率、'
          '烹飪與力矩等更多類別。\n\n'
          '{ } 括號始終顯示另一個系統中的對應值。多個單位可以組合成'
          '一個數值——例如 1 ft 6 in 或 2 h 30 min。按下 = 可循環'
          '切換單位表示法與逐級細分；長按某個單位則顯示簡短定義。'
          '數字以十二進位字形或一般的 0–9/A/B 顯示，依顯示設定而定。',
    ),
    UnitTheorySection(
      '換算器不能做什麼',
      '它是換算器，而非科學計算機：沒有正弦或對數等函數，'
          '不能用 × 與 ÷ 自由運算，也沒有記憶與角度模式。'
          '這些功能請使用主計算機。\n\n'
          '貨幣（便士／先令／英鎊）反映的是英國歷史上的換算比例——'
          '十二便士等於一先令，二十先令等於一英鎊——而非即時匯率。'
          '它展示的是舊制英國貨幣的十二進位結構，而非當日的貨幣價值。'
          '單位組合是固定預設的。',
    ),
    UnitTheorySection(
      '如何操作',
      '點選一個類別（例如長度）——其單位序列便會在對側欄位展開。'
          '點選一個單位、輸入數字，再把多個單位組合成一個數值。'
          'DOZ/DEZ 鍵在英制世界與公制世界之間切換；{ } 括號顯示對應值。'
          '= 循環切換顯示方式，長按單位可查看其含義，點選輸入列'
          '則可移動游標。\n\n'
          '舉一個例子——輸入 1 ft 6 in，然後在對側世界讀取結果：',
    ),
  ];
}
  ...
  switch (cat) {
    case UnitCategory.count:
      return const [
        UnitTheorySection(
          '以十二為單位疊加計數',
          '計數可以十個一組捆綁——也可以十二個一組。一打是十二件，'
              '一籮是十二打（144），一大籮是十二籮（1728）。這與個、十、百、'
              '千的階梯完全相同，只是改用十二進位：一打 = 12¹，'
              '一籮 = 12²，一大籮 = 12³。用十二進位書寫，分別是整齊的'
              '10、100 與 1000——因此大籮就是「十二進位的千」。\n\n'
              '為什麼十二如此便於整除，以及如何用四根手指的三個指節數到十二，'
              '請見「理論 → 基礎」。本文專注於計數單位本身。',
        ),
        UnitTheorySection(
          '名稱的由來',
          '「打」（dozen）經由古法語 douzaine 源自拉丁語 duodecim，'
              '意為「二加十」；這個詞精確地指十二，並非大約。'
              '西班牙語 docena、義大利語 dozzina 與英語 dozen 均與此相關。\n\n'
              '「籮」（gross）源自 grosse douzaine，意為「大打」，'
              '最早見於 15 世紀初。批發商以籮為單位計點並包裝小商品——'
              '鉛筆、鈕扣、針——有些鉛筆盒至今仍印著「1 gross」。'
              '大籮（1728）是十二籮——同時也恰好是一立方呎中立方吋的數目'
              '（12 × 12 × 12）。雞蛋至今仍以打出售。',
        ),
        UnitTheorySection(
          '烘焙師的一打',
          '烘焙師的一打是十三個——比普通一打多一個；第十三個麵包叫做'
              '「vantage loaf」（盈利麵包）。常見的解釋指向中世紀英格蘭的'
              '麵包法令（Assize of Bread，13 世紀）：販售不足重量的麵包'
              '面臨罰則，因此烘焙師為保險起見多放一個。\n\n'
              '此處需要一個誠實的保留：這個說法有其道理且廣泛流傳，'
              '但並無文獻記載可以佐證。英語「baker’s dozen」最早的'
              '書面記錄出現在 16 世紀末——遠在該法令之後。',
        ),
      ];
