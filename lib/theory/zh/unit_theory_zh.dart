...
List<UnitTheorySection> _converterIntroZh() {
  return const [
    UnitTheorySection(
      '换算器能做什么',
      '单位换算器在英制十二进制世界（底数十二）与公制世界（底数十）之间'
          '进行换算——涵盖所有类别：长度、面积、体积、重量、时间、角度、'
          '货币、温度，以及压力、力、能量、功率、烹饪和扭矩等更多类别。\n\n'
          '{ } 括号始终显示对应另一体系中的数值。多个单位可以组合成一个数值——'
          '例如 1 ft 6 in 或 2 h 30 min。按 = 可循环切换各种单位显示形式和'
          '逐级分解；长按某个单位则显示其简短定义。数字会以十二进制字形或'
          '普通的 0–9/A/B 显示，取决于显示设置。',
    ),
    UnitTheorySection(
      '换算器不能做什么',
      '它是一个换算器，而非科学计算器：没有正弦或对数之类的函数，'
          '不能用 × 和 ÷ 自由运算，没有内存，也没有角度模式。这些功能'
          '请使用主计算器。\n\n'
          '货币（便士／先令／英镑）反映的是历史上的英国比例——十二便士兑一先令，'
          '二十先令兑一英镑——而非当前汇率。它展示的是旧货币的十二进制结构，'
          '而非今日的实际货币价值。可用的单位集合是固定的。',
    ),
    UnitTheorySection(
      '如何使用',
      '点击一个类别（例如长度）——其单位阶梯便在对面一列展开。点击某个单位，'
          '输入数字，再将多个单位组合成一个数值。DOZ/DEZ 键在英制世界和公制世界'
          '之间切换；{ } 括号显示对应的值。= 循环切换各种显示形式，长按某个单位'
          '显示其含义，点击输入行则移动光标。\n\n'
          '举个例子——输入 1 ft 6 in，并在另一世界中读取结果：',
    ),
  ];
}
